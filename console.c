...
static int aptina_command( int argc, char* argv[] );
...
static const command_t commands[] = {
    ...
    { (char*) "apt",                 aptina_command,              0, DELIMIT, NULL, NULL, NULL},
#if 0
    WIFI_COMMANDS
    PING_COMMANDS
    WPS_COMMANDS
...
#endif
int aptina_command( int argc, char* argv[] ) {
    AppDbg_Printf("\r\nAPTINA COMMANDS");
    return 0;
}
...
int help_command( int argc, char* argv[] )
{
    ...
    switch ( argc )
    {
        case 0:
        case 1:
            AppDbg_Printf( "Console Commands:\r\n" );
            for ( cmd_ptr = console_command_table; cmd_ptr->name != NULL; cmd_ptr++ )
            {
        ...
        case 2:
            ...
            for ( cmd_ptr = console_command_table; cmd_ptr->name != NULL; cmd_ptr++ )
            {
                if ( strcmp( argv[1], cmd_ptr->name ) == 0 )
                {
                    ...
                    {
                        err = ERR_CMD_OK;
                        AppDbg_Printf( "No example available for %s\r\n\r\n", argv[1] );
                    }
                }
            }
            break;
    ...
    else
    {
        ...
        for ( cmd_ptr = console_command_table; cmd_ptr->name != NULL; cmd_ptr++ )
        {
            if ( ( strncmp( cmd_ptr->name, console_buffer, buf_len ) == 0 ) && ( strcmp( cmd_ptr->name, "" ) != 0 ) )
            {
                ...
                AppDbg_Printf( "%-19.19s", cmd_ptr->name );
                cnt++;
            }
        }
        if ( cnt )
...
cmd_err_t console_process_char( char c )
{
    ...
    /* AppDbg_Printf("ConsoleProcessChar\r\n"); */
